 ...
 * @since 2018-05-29
 */
public class Encrypt implements Mission<byte[]>{
	...
	public Encrypt(String password, String initialVector) {
	...
	@Override
	public byte[] execute(byte[] input) throws NoSuchAlgorithmException, InvalidKeyException {
		try {
			SecretKeySpec key = createKey();
			Cipher cipher = Cipher.getInstance(mAlgorithm);
			cipher.init(Cipher.ENCRYPT_MODE, key, createInitialVector());
			byte[] byteContent = input;
